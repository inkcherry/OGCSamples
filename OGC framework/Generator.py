...
class Manager(nn.Module):
    ...
    def forward(self, f_t, h_m_t, c_m_t):
        ...
        sub_goal = torch.renorm(sub_goal, 2, 0,
                                1.0)  # Returns a tensor where each sub-tensor of input along dimension dim is normalized such that the p-norm of the sub-tensor is lower than the value maxnorm
...
class Worker(nn.Module):
    ...
    def forward(self, x_t, h_w_t, c_w_t):
        ...
        x_t=x_t.detach()
        h_w_t=h_w_t.detach()
        c_w_t=c_w_t.detach()  #detach h,c perform not well
...
class Generator(nn.Module):
    ...
    def forward(self, x_t, f_t, h_m_t, c_m_t, h_w_t, c_w_t, last_goal, real_goal, t, temperature):
        # A goal(for caculate loss with delta feature) = last_goal+sub_goal+sub_goal+...+sub_goal,last goal
        #real_goal is constan input  to caculate probs, when a run a manager step_size and real_goal =last_goal , detach also help recurrent a new goal.

        if(not t==0):  #t=0 ,don't detach, to get grad of para of  goal_init
            real_goal=real_goal.detach()
        ...
        #for work,all real_goal detach()
        w_t = torch.matmul(
            real_goal.detach().fill_(1), self.worker.goal_change
        )
        w_t = torch.renorm(w_t, 2, 0, 1.0)
        w_t = torch.unsqueeze(w_t, -1)
        logits = torch.squeeze(torch.matmul(output, w_t))
        # logitss=F.sigmoid(logits)
        probs = F.softmax(temperature * logits, dim=1)
        probs[:,32]=0
        # probsnp=probs.detach().cpu.numpy()    
        # probnp=probs.detach().cpu().numpy()
        # cc=np.max(probnp,1)
