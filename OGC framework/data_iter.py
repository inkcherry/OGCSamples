...
class Dis_norrow_Dataset(Dataset):
    def __init__(self, positive_filepath, negative_filepath):
        pos_data = np.load(positive_filepath, allow_pickle=True)
        neg_data = np.load(negative_filepath, allow_pickle=True)
...
def dis_narrow_data_loader(positive_filepath, negative_filepath, batch_size, shuffle, num_workers, pin_memory):
    dataset = Dis_norrow_Dataset(positive_filepath, negative_filepath)
    return DataLoader(dataset=dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, pin_memory=pin_memory)
#just for test reward
# def dis_narrow_data_loader_without_shuffle(positive_filepath, negative_filepath, batch_size, shuffle, num_workers, pin_memory):
#     dataset = Dis_norrow_Dataset(positive_filepath, negative_filepath)
#     return DataLoader(dataset=dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=pin_memory)
